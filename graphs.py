...
def make_agg_network_graph(df):
    # Step 1: Parse `metadata.relatedCards` into a list of IDs
    def parse_related_cards(related_cards):
        if isinstance(related_cards, str):
            # try:
            related_cards = json.loads(related_cards)
        # except json.JSONDecodeError:
        # return []
        if isinstance(related_cards, dict):
            return list(related_cards.keys())
        return []

    df["metadata.relatedCards"] = df["metadata.relatedCards"].apply(parse_related_cards)
