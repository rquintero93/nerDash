...
def main():
    ...
    with col5:
        ...
        createdAt_timeline_chart = make_line_chart(data=df_cards, 
                                                   y="createdAt", x="_id")
        st.plotly_chart(createdAt_timeline_chart, use_container_width=True)

    with col6:
        st.subheader("Concepts Over Time")
        updatedAt_timeline_chart = make_line_chart(data=df_cards, 
                                                   y="updatedAt", x="name")
    ...
    with col9:
        ...
        filtered_name_counter = dict(sorted(name_counter.items(), 
                                        key=lambda item: item[1], reverse=True)[:20])
        name_counter_bar_chart = make_bar_chart(data=filtered_name_counter,
                                                orientation="h")
